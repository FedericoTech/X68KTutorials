...
__inline void init_system()
{
    signal(SIGINT, shut_system);	/* Processing routine settings when is pressed CTRL-C */
    signal(SIGTERM, shut_system);
...
char * loadData(char * buffer, const char * filename, int8_t config)
{
    ...
    if(file_handler < 0){
        ...
        _dos_c_print("\r\n");
        _dos_c_print(getFileErrorMessage(file_handler));
    ...
    read = _dos_read(file_handler, buffer, status);
    ...
    if(status < 0){
        ...
        _dos_c_print(getFileErrorMessage(status));
        _dos_exit2(status);
    }
